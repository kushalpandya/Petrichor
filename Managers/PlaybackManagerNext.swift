...
// seeking, and playback state management using AudioStreaming (AVAudioEngine).
//
...
class PlaybackManager: NSObject, ObservableObject {
    ...
    private var pauseHibernationTimer: Timer?
    ...
    func togglePlayPause() {
        if isPlaying {
            ...
            // Start hibernation countdown after pause
            startPauseHibernationTimer()
        } else {
            // Stop hibernation timer since we're resuming
            stopPauseHibernationTimer()
            
            // Check if we need to wake from hibernation
            if isHibernating {
                wakeFromHibernation()
            }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    self.lastReportedTime = position
    ...
    private func restartPlayback(from fullTrack: FullTrack) {
        // If we're hibernating, wake up first
        if isHibernating {
            wakeFromHibernation()
        }
        ...
        audioPlayer.play(url: fullTrack.url)
        
        if targetPosition > 0 {
            isSeeking = true
            seekTargetTime = targetPosition
    ...
    private func startPauseHibernationTimer() {
        stopPauseHibernationTimer()
        
        pauseHibernationTimer = Timer.scheduledTimer(withTimeInterval: TimeConstants.pauseHibernationThreshold, repeats: false) { [weak self] _ in
            self?.enterHibernation()
        }
    }

    private func stopPauseHibernationTimer() {
        pauseHibernationTimer?.invalidate()
        pauseHibernationTimer = nil
    }
    ...
    private func enterHibernation() {
        ...
        // Stop and discard AudioPlayer
        audioPlayer.stop()
        
        // Mark as hibernating
        isHibernating = true
        
        Logger.info("Playback paused for over \(TimeConstants.pauseHibernationThreshold) seconds, entering hibernation")
    }
...
extension PlaybackManager: AudioPlayerDelegate {
    ...
    func audioPlayerDidFinishBuffering(player: AudioPlayer, with entryId: AudioEntryId) {
        // Unused, but required for AudioPlayerDelegate conformance
    }
